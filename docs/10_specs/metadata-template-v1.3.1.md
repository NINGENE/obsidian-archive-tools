...
- Obsidian Basesでのサムネイル表示は、`thumbnail` プロパティをImage propertyに指定する
- `thumbnail` パスはクォートなしで記述する
- `cover` プロパティとBases formulaは予備案として保留する
...
Obsidian Basesでは、この `thumbnail` プロパティをImage propertyに指定することで、カードや一覧にサムネイルを表示できる。
...
上記はBasesで表示されなかったためNG。

Pythonでメディアノートを自動生成する場合も、`thumbnail` はクォートなしで出力する。
...
````
v1.3の実検証では、`thumbnail` プロパティをBasesのImage propertyに指定することで、複数画像のサムネイル一覧表示ができた。

そのため、v1.3では `cover` プロパティは使わない。

また、当初検討していたBases formula方式も、現時点では不要と判断する。必要になった場合の予備案として保留する。
...
Basesで画像表示するため、`thumbnail` パスはクォートなしで記述する。

### 表示モードに関するメモ

Obsidian上では、BasesとNotes Explorerで表示の思想が異なる。

```
Bases:
  thumbnailプロパティの画像を表示する
  タグやプロパティを一緒に表示できる
  ノート本文は見えない

Notes Explorer:
  ノートそのものをサムネイル・プレビューとして表示する
```

将来自作アプリでは、以下のような表示モード切り替えを検討する。
